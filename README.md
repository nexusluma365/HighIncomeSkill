...
`STRIPE_SECRET_KEY` is required in Netlify. `STRIPE_PUBLISHABLE_KEY` is optional because the app includes the current live publishable key as a safe client-side fallback. Set `STRIPE_PUBLISHABLE_KEY` only if you want to override it.
